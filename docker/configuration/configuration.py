...
if "ENFORCE_GLOBAL_UNIQUE" in environ:
    ENFORCE_GLOBAL_UNIQUE = _environ_get_and_map(
        "ENFORCE_GLOBAL_UNIQUE", None, _AS_BOOL
    )
...
# LOGIN_REQUIRED is deprecated in NetBox 4.x and removed in NetBox 5.x.
# Intentionally ignore any LOGIN_REQUIRED environment variable.
...
if "POWERFEED_DEFAULT_AMPERAGE" in environ:
    POWERFEED_DEFAULT_AMPERAGE = _environ_get_and_map(
        "POWERFEED_DEFAULT_AMPERAGE", None, _AS_INT
    )
...
if "POWERFEED_DEFAULT_VOLTAGE" in environ:
    POWERFEED_DEFAULT_VOLTAGE = _environ_get_and_map(
        "POWERFEED_DEFAULT_VOLTAGE", None, _AS_INT
    )
