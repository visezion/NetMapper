"""Ingestor for netmiko_hp_procurve_show_arp."""
__author__ = "Andrea Dainese"
__contact__ = "andrea@adainese.it"
__copyright__ = "Copyright 2022, Andrea Dainese"
__license__ = "GPLv3"

from netmapper.schemas import interface, arptableentry
...
def ingest(log):
    ...
    for item in log.parsed_output:
        # See https://github.com/networktocode/ntc-templates/tree/master/tests/hp_procurve/show_arp # pylint: disable=line-too-long
        if utils.incomplete_mac(item.get("mac_address")):
            continue
        interface_name = item.get("port")
        if not interface_name:
            # Skip ARP without interface
            continue
