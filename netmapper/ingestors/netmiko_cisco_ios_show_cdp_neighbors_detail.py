"""Ingestor for netmiko_cisco_ios_show_cdp_neighbors_detail."""
...
from netmapper.schemas import interface, device, discoverable, cable
...
def ingest(log):
    ...
    for item in log.parsed_output:
        # See https://github.com/networktocode/ntc-templates/tree/master/tests/cisco_ios/show_cdp_neighbors_detail # pylint: disable=line-too-long
        ...
        remote_management_ip = utils.normalize_ip_address_or_none(
            item.get("management_ip") or item.get("mgmt_address")
        )
        remote_name = utils.get_remote_device_name(
            item.get("destination_host") or item.get("neighbor_name"),
