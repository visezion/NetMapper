...
class ArpTableEntryTable(NetBoxTable):
    ...
    mac_address = tables.LinkColumn(
        "plugins:netmapper:arptableentry",
...
class DiscoverableTable(NetBoxTable):
    ...
    discovery_button = """
    <a class="btn btn-sm btn-secondary" href="{% url 'plugins:netmapper:discoverable_discover' pk=record.pk %}" title="Discover">
...
class MacAddressTableEntryTable(NetBoxTable):
    ...
    mac_address = tables.LinkColumn(
        "plugins:netmapper:macaddresstableentry", args=[tables.utils.A("pk")]
    )
