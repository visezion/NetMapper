...
def find_model(manufacturer=None, keyword=None):
    ...
    netmapper_directory = path.dirname(path.realpath(__file__))
    library_file = f"{netmapper_directory}/library/{manufacturer}.yml"
...
def find_vendor(keyword=None):
    ...
    """
    netmapper_directory = path.dirname(path.realpath(__file__))
    library_file = f"{netmapper_directory}/library"
...
def is_command_supported(discoverer, command):
    ...
    try:
        importlib.import_module(f"netmapper.ingestors.{function_name}")
...
def log_ingest(log):
    ...
    try:
        module = importlib.import_module(f"netmapper.ingestors.{function_name}")
...
def spawn_script(script_name, get_data=None, post_data=None, file_list=None, user=None):
    ...
    from netmapper import sync_plugin_assets  # pylint: disable=import-outside-toplevel
    ...
    try:
        _, script = get_module_and_script("netmapper_jobs", script_name)
    except (
